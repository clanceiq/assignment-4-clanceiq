...
impl<T> List<T> {
    ...
    fn append(&mut self, data: T) {
        ...
        if let Some(ref mut last) = self.last { // gets the old last node if it exists
            
            if let Some(new_last) = Node::append(last, data) { // creates a new last node and appends data to it
                self.last = Some(new_last); // sets the new last as the last value in the list
            }
        } else { // handles empty list case
